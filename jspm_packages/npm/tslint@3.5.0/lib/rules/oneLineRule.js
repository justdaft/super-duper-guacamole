...
var OneLineWalker = (function(_super) {
  ...
  OneLineWalker.prototype.visitCatchClause = function(node) {
    var catchClosingParen = node.getChildren().filter(function(n) {
      return n.kind === ts.SyntaxKind.CloseParenToken;
    })[0];
  ...
  OneLineWalker.prototype.visitTryStatement = function(node) {
    ...
    var finallyKeyword = node.getChildren().filter(function(n) {
      return n.kind === ts.SyntaxKind.FinallyKeyword;
    })[0];
  ...
  OneLineWalker.prototype.visitVariableDeclaration = function(node) {
    ...
    if (initializer != null && initializer.kind === ts.SyntaxKind.ObjectLiteralExpression) {
      var equalsToken = node.getChildren().filter(function(n) {
        return n.kind === ts.SyntaxKind.EqualsToken;
      })[0];
...
function getFirstChildOfKind(node, kind) {
  return node.getChildren().filter(function(child) {
