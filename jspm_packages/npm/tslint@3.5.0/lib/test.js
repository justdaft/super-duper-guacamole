...
(function(process) {
  ...
  function runTest(testDirectory) {
    ...
    var tslintConfig = JSON.parse(fs.readFileSync(path.join(testDirectory, "tslint.json"), "utf8"));
    var results = {
      directory: testDirectory,
      results: {}
    };
    for (var _i = 0,
        filesToLint_1 = filesToLint; _i < filesToLint_1.length; _i++) {
  ...
  function consoleTestResultHandler(testResult) {
    ...
    for (var _i = 0,
        _a = Object.keys(testResult.results); _i < _a.length; _i++) {
      var fileName = _a[_i];
      ...
      var didTestPass = !diffResults.some(function(diff) {
      ...
      if (didTestPass) {
        ...
        for (var _b = 0,
            diffResults_1 = diffResults; _b < diffResults_1.length; _b++) {
