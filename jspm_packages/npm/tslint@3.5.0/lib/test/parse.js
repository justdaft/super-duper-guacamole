...
function removeErrorMarkup(text) {
  ...
  var codeText = lines.filter(function(line) {
    return (line instanceof lines_1.CodeLine);
  }).map(function(line) {
...
function parseErrorsFromMarkup(text) {
  ...
  var messageSubstitutionLines = lines.filter(function(l) {
    return l instanceof lines_1.MessageSubstitutionLine;
  });
  var messageSubstitutions = {};
  for (var _i = 0,
      messageSubstitutionLines_1 = messageSubstitutionLines; _i < messageSubstitutionLines_1.length; _i++) {
...
function createMarkupFromErrors(code, lintErrors) {
  ...
  var errorLinesForCodeText = codeText.map(function() {
    return [];
  });
  for (var _i = 0,
      lintErrors_1 = lintErrors; _i < lintErrors_1.length; _i++) {
    var error = lintErrors_1[_i];
    var startPos = error.startPos,
        endPos = error.endPos,
...
function combineCodeTextAndErrorLines(codeText, errorLinesForCodeText) {
  return codeText.reduce(function(resultText, code, i) {
    ...
    resultText.push.apply(resultText, (errorLinesForCodeText[i].map(function(line) {
      return lines_1.printLine(line, code);
    })));
    return resultText;
  }, []);
}
function createCodeLineNoToErrorsMap(lines) {
  ...
  for (var _i = 0,
      lines_2 = lines; _i < lines_2.length; _i++) {
    var line = lines_2[_i];
...
function isValidErrorMarkupContinuation(errorLinesForCodeLines, lineNo) {
  return lineNo < errorLinesForCodeLines.length && errorLinesForCodeLines[lineNo].length !== 0 && errorLinesForCodeLines[lineNo][0].startCol === 0;
}
