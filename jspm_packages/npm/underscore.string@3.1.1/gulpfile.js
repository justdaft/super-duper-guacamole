...
(function(process) {
  ...
  gulp.task('test', ['browserify'], function(cov) {
    ...
    return gulp.src(['*.js', 'helper/*.js']).pipe(istanbul()).pipe(istanbul.hookRequire()).on('finish', function() {
      return gulp.src(['tests/*.js']).pipe(mocha({
        ui: 'qunit',
        reporter: 'dot'
      })).pipe(istanbul.writeReports({reporters: reporters}));
    });
  });
  gulp.task('bench', ['browserify'], function(func) {
    func = func || '*';
    return gulp.src('bench/' + func + '.js').pipe(bench());
  });
  gulp.task('browserify', function() {
    return gulp.src(SRC).pipe(browserify({
      detectGlobals: true,
      standalone: 's'
    })).pipe(rename('underscore.string.js')).pipe(gulp.dest(DEST));
  });
  gulp.task('clean', function() {
    return gulp.src(DEST).pipe(rimraf());
  });
  gulp.task('bump-in-js', function(semver) {
    return gulp.src(VERSION_FILES_JS).pipe(replace(/(version?\s?=?\:?\s\')([\d\.]*)\'/gi, '$1' + semver + "'")).pipe(gulp.dest('./'));
  });
  gulp.task('bump', ['bump-in-js'], function(semver) {
    ...
    return gulp.src(VERSION_FILES).pipe(bump({version: semver})).pipe(gulp.dest('./'));
  });
  gulp.task('build', ['test', 'clean'], function() {
    gulp.src(DEST + '/' + SRC_COMPILED).pipe(uglify()).pipe(rename(MIN_FILE)).pipe(gulp.dest(DEST));
  });
})(require('process'));
